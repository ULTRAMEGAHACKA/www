...
@app.route('/index')
def index():
    imgs = ['nesterov.png', 'bash.png']
    subjs = ["Categories", "City", "Rating"]
    ...
    return render_template('index.html', title='Find Hotels', pname='HOME', navmenu=controller.navmenu,
                           imgs=imgs, subjs=subjs, processed_files=processed_files)

# Обработка запроса к странице contact.html
@app.route('/contact')
def contact():
    return render_template('contact.html', title='Find Hotels', pname='CONTACT', navmenu=controller.navmenu)

@app.route('/rating')
def rating():
    return render_template('rating.html', title='Find Hotels', pname='Rating', navmenu=controller.aside_menu)
